...
class SimpleTiledModel : Model
{
    ...
    public SimpleTiledModel(string name, string subsetName, int width, int height, bool periodic, bool blackBackground, Heuristic heuristic) : base(width, height, 1, periodic, heuristic)
    {
        ...
        XElement xroot = XDocument.Load($"WFC/tilesets/{name}.xml").Root;
        ...
        foreach (XElement xtile in xroot.Element("tiles").Elements("tile"))
        {
            ...
            if (unique)
            {
                ...
                {
                    int[] bitmap;
                    (bitmap, tilesize, tilesize) = BitmapHelper.LoadBitmap($"WFC/tilesets/{name}/{tilename} {t}.png");
                    tiles.Add(bitmap);
            ...
            {
                int[] bitmap;
                (bitmap, tilesize, tilesize) = BitmapHelper.LoadBitmap($"WFC/tilesets/{name}/{tilename}.png");
                tiles.Add(bitmap);
